module Unidad
    def descansar # Corta la recursividad
        #No hace nada
    end


module Atacante # Mixin de Atacante por es el module
  ...
  def descansar
    super() # Llama al anteriori pude ser Defensor o Unidad
    self.descansado = true
  end

end

module Defensor # Mixin de Defensor por eso el module
...
# Se resuelve con G -> D -> U(Porque lo decide ruby se elimina) -> A -> U -> O
class Guerrero 
  include Atacante
  # alias_method :descansar_atacante, :descansar Metodo fisura
  # Es uan form de metoprogramar esto ya que le paso la variable donde va a guardarle y le asigna la viariable al emtodo 
  # con un get y set de un atrivuto y este se le asigna a la variable descansar_atacante
  include Defensor # Este es el que pisa ya que se implemetna despues de Atacante
  # Por linializacion Defensor es mas importnate que Atacante
  ...
  def descansar
    super()
    # descansar_atacante forna de usar de forma fisura 
  end
  ...
  def recibi_danio(danio)
    super(danio) # Ya llama al super del mismo metodo pero del padre
    self.peloton.recibi_danio()
  end

end

class Espadachin < Guerrero # Heradando de guerrero
    # [Espadacin(No varia), Guerrero(Pueden variar), Objeto(No varia), mixines(Pueden variar), Kernel(No varia), BasicObjet(No varia) ] 
...
class Misil
  include Unidad #Cortas la recusivada para que no rompa todo a la mierda
  include Atacante
